...
import { connectNATS, createPipeVistaEvent, publishEvent, createStream } from '@vistafam/pipevista-core';
...
const workflowTriggers = new Map();
...
app.post('/v1/events/dlq/:id/replay', async (req, reply) => {
    ...
    if (!entry) {
        reply.status(404).send({ error: 'DLQ entry not found' });
        return;
    }
    dlq.delete(id);
...
async function start() {
    ...
    try {
        ...
        logger.fatal('Failed to start Event Hub', { error: err.message });
        process.exit(1);
    }
}
