...
const webhooks = new Map<string, { id: string; tenantId: string; url: string; events: string[]; secret: string; active: boolean }>();
const circuitBreakers = new Map<string, CircuitBreaker>();

function getCB(name: string): CircuitBreaker {
  if (!circuitBreakers.has(name)) circuitBreakers.set(name, new CircuitBreaker({ name, failureThreshold: 5, timeoutMs: 60000 }));
...
app.post('/v1/connectors/webhooks', async (req, reply) => {
  const body = req.body as { tenantId: string; url: string; events: string[]; secret: string };
...
app.post('/v1/connectors/webhooks/:id/dispatch', async (req, reply) => {
  ...
  if (!webhook || !webhook.active) { reply.status(404).send({ error: 'Webhook not found or inactive' }); return; }

  const body = req.body as { eventType: string; payload: unknown };
...
app.post('/v1/connectors/proxy', async (req, reply) => {
  const body = req.body as { method: string; url: string; headers?: Record<string, string>; body?: unknown; timeout?: number };
  try {
...
async function start() {
  ...
  try {
    ...
    logger.fatal('Failed to start Connector', { error: (err as Error).message });
    process.exit(1);
  }
}
