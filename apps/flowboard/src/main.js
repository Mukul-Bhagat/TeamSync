...
async function start() {
    ...
    try {
        ...
        await rebuildTriggerCache(triggers.map((t) => ({
            triggerId: t.id,
        ...
        logger.warn('Failed to rebuild trigger cache at startup', { error: err.message });
    }
    // Start workers
    ...
    try {
        ...
        logger.fatal('Failed to start FlowBoard', { error: err.message });
        process.exit(1);
    }
}
start();
