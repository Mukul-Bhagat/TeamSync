...
export async function executionRoutes(app: FastifyInstance) {
  ...
  app.get('/v1/executions', async (req) => {
    const { tenantId, workflowId, status } = req.query as {
      tenantId?: string;
      workflowId?: string;
      status?: string;
    };
  ...
  app.get('/v1/executions/:id', async (req, reply) => {
    ...
    if (!execution) { reply.status(404).send({ error: 'Execution not found' }); return; }
    return { execution };
  });
  ...
  app.post('/v1/workflows/:workflowId/execute', async (req, reply) => {
    ...
    if (workflow.status !== 'active') { reply.status(400).send({ error: 'Workflow is not active' }); return; }
  ...
  app.get('/v1/executions/:id/timeline', async (req, reply) => {
    ...
    ].sort((a: any, b: any) => new Date(a.createdAt ?? a.startedAt).getTime() - new Date(b.createdAt ?? b.startedAt).getTime());
