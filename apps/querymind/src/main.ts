...
app.post('/api/v1/translate', async (req, reply) => {
  const body = req.body as { query: string; schema: string };
...
app.post('/api/v1/execute', async (req, reply) => {
  const body = req.body as { sql: string; tenantId: string };
...
async function start() {
  ...
  try {
    ...
    logger.fatal('Failed to start QueryMind', { error: (err as Error).message });
    process.exit(1);
  }
}
