...
app.post('/api/v1/schemas', async (req, reply) => {
  const body = req.body as { name: string; tables: unknown[] };
...
app.get('/api/v1/schemas/:id', async (req, reply) => {
  ...
  if (!schema) { reply.status(404).send({ error: 'Schema not found' }); return; }
  reply.send(schema);
});
...
app.post('/api/v1/migrations/plan', async (req, reply) => {
  const body = req.body as { fromSchemaId: string; toSchemaId: string };
  reply.send({
    plan: [
...
async function start() {
  ...
  try {
    ...
    logger.fatal('Failed to start SchemaForge', { error: (err as Error).message });
    process.exit(1);
  }
}
