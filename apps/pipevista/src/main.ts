...
app.post('/registry/register', async (req, reply) => {
  const body = req.body as { id: string; name: string; host: string; port: number };
...
app.post('/registry/heartbeat/:id', async (req, reply) => {
  ...
  registry.set(id, { ...service as object, lastHeartbeat: new Date().toISOString() });
...
app.get('/api/v1/health/system', async () => ({
  overall: 'healthy',
  services: Array.from(registry.values()).map((s: any) => ({
    name: s.name,
...
async function start() {
  ...
  try {
    ...
    logger.fatal('Failed to start PipeVista', { error: (err as Error).message });
    process.exit(1);
  }
}
