    ...
    return (<div className="flex h-full">
      <div className="w-64 border-r border-white/5 bg-[#080808]/40 flex flex-col">
        ...
        <div className="flex-1 overflow-auto p-2 space-y-1">
          {channels?.map((channel) => (<button key={channel.id} onClick={() => setSelectedChannel(channel)} className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-all ${selectedChannel?.id === channel.id
      ...
      <div className="flex-1 flex flex-col">
        {selectedChannel && (<>
            ...
            <div ref={scrollRef} className="flex-1 overflow-auto p-6 space-y-6 scrollbar-thin">
              {messages?.length === 0 && (<div className="flex flex-col items-center justify-center py-20 text-center opacity-50">
                  <Hash className="h-12 w-12 mb-4 text-white/20"/>
                  <p className="text-sm font-medium">This is the start of #{selectedChannel.name}</p>
                </div>)}
              {messages?.map((message) => (<motion.div key={message.id} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} className="flex items-start gap-3">
            ...
            }} placeholder={`Message #${selectedChannel.name}`} className="glass-input flex-1"/>
                <button onClick={() => inputValue.trim() && sendMessage.mutate(inputValue)} disabled={!inputValue.trim() || sendMessage.isPending} className="h-10 w-10 bg-primary text-primary-foreground rounded-lg flex items-center justify-center hover:bg-primary/90 disabled:opacity-50 transition-colors">
