...
export function KnowledgeView() {
    ...
    return (<div className="flex h-full">
        ...
        <div className="p-3 space-y-1">
          {categories?.map((category) => (<div key={category.id}>
              ...
              {pages
                ...
                .map((page) => (<button key={page.id} onClick={() => setSelectedPage(page)} className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-all ${selectedPage?.id === page.id
      ...
      <div className="flex-1 overflow-auto p-8 scrollbar-thin">
        {selectedPage ? (<motion.div key={selectedPage.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-3xl">
          ...
          </motion.div>) : (<div className="h-full flex flex-col items-center justify-center text-center">
