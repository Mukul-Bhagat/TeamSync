...
export function createAISummaryWorker(concurrency = 2): Worker {
  return new Worker<AISummaryJob>(
    ...
    async (job: Job<AISummaryJob>) => {
      ...
      let messages: { content: string; sender_name: string; created_at: string }[] = [];
      ...
      try {
        ...
        summary = (res.data as { choices: { message: { content: string } }[] }).choices[0]?.message?.content ?? '';
      } catch (err) {
        logger.error('AI summary generation failed', { error: (err as Error).message });
