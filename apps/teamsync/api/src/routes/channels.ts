...
export async function channelRoutes(app: FastifyInstance) {
  ...
  app.post('/v1/channels', async (req, reply) => {
    ...
    if (error || !channel) { reply.status(500).send({ error: error?.message ?? 'Failed' }); return; }
  ...
  app.get('/v1/channels/:id', async (req, reply) => {
    ...
    if (error || !data) { reply.status(404).send({ error: 'Channel not found' }); return; }
  ...
  app.patch('/v1/channels/:id', async (req, reply) => {
    const { id } = req.params as { id: string };
    const body = req.body as Partial<{ name: string; description: string; is_private: boolean }>;
    const supabase = getSupabase();
  ...
  app.post('/v1/channels/:id/members', async (req, reply) => {
    const { id } = req.params as { id: string };
    const body = req.body as { user_id: string; role?: string };
    const supabase = getSupabase();
