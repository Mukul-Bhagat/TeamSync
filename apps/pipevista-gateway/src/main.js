...
import { createRedisClient, buildKey, KeyPrefixes, slidingWindowCheck } from '@vistafam/pipevista-core';
...
async function start() {
        ...
        await app.register(httpProxy, {
            ...
            replyOptions: {
                rewriteRequestHeaders: (req, headers) => {
                    headers['x-trace-id'] = req.headers['x-trace-id'] || req.id;
                    headers['x-tenant-id'] = req.headers['x-tenant-id'] || 'default';
                    return headers;
                },
            },
    ...
    app.setErrorHandler((error, req, reply) => {
        logger.error('Gateway error', { url: req.url, error: error.message });
