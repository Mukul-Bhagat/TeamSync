...
function channelRoom(tenantId: string, channelId: string) { return `tenant:${tenantId}:channel:${channelId}`; }
function userRoom(tenantId: string, userId: string) { return `tenant:${tenantId}:user:${userId}`; }
function presenceRoom(tenantId: string) { return `tenant:${tenantId}:presence`; }
...
app.post('/v1/realtime/presence', async (req, reply) => {
  const body = req.body as { tenantId: string; userId: string; status: 'online' | 'away' | 'dnd' | 'offline'; platform?: string };
...
app.post('/v1/realtime/notify', async (req, reply) => {
  const body = req.body as { tenantId: string; userIds?: string[]; channelId?: string; eventName: string; payload: unknown };
...
app.post('/v1/realtime/bridge', async (req, reply) => {
  const body = req.body as { subject: string; tenantId: string; payload: unknown };
...
async function start() {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('authenticate', async (data: { tenantId: string; userId: string }) => {
    ...
    socket.on('join:channel', (data: { tenantId: string; channelId: string }) => {
      socket.join(channelRoom(data.tenantId, data.channelId));
      socket.emit('channel:joined', { channelId: data.channelId });
    });

    socket.on('leave:channel', (data: { tenantId: string; channelId: string }) => {
  ...
  try {
    ...
    logger.fatal('Failed to start Realtime', { error: (err as Error).message });
    process.exit(1);
  }
}
