...
import { createRedisClient, buildKey, KeyPrefixes } from '@vistafam/pipevista-core';
...
function getCB(name) {
    if (!circuitBreakers.has(name))
        circuitBreakers.set(name, new CircuitBreaker({ name, failureThreshold: 3, timeoutMs: 60000 }));
...
async function start() {
    ...
    try {
        ...
        logger.fatal('Failed to start AI Router', { error: err.message });
        process.exit(1);
    }
}
