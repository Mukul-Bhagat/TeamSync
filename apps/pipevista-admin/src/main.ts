...
const serviceStatuses = new Map<string, { status: string; lastUpdated: string; version: string }>();
...
app.put('/v1/admin/config/:key', async (req, reply) => {
  ...
  const body = req.body as { value: unknown; scope?: string };
...
app.post('/v1/admin/commands/:command', async (req, reply) => {
  ...
  const body = req.body as { targetService?: string; parameters?: Record<string, unknown> };
...
async function start() {
  ...
  try {
    ...
    logger.fatal('Failed to start Admin', { error: (err as Error).message });
    process.exit(1);
  }
}
