 ...
 */
import { connect, JSONCodec, headers, StringCodec } from 'nats';
...
// ── Event Factory ──────────────────────────────────────────
export function createPipeVistaEvent(request) {
    return {
        id: randomUUID(),
...
export async function requestReply(subject, event, timeoutMs = 5000) {
    const conn = getNATSConnection();
    event.subject = subject;
    const h = headers();
    h.append('trace-id', event.traceId);
...
export async function subscribeToEvents(subject, handler, options) {
    const conn = getNATSConnection();
...
export async function createConsumer(streamName, consumerName, subjectFilter, options) {
    const conn = getNATSConnection();
    const jsm = await conn.jetstreamManager();
    try {
